...
graph TD
    ...
    %% Jetson 计算节点
    subgraph Compute_Node ["🚀 Jetson Orin Compute Node"]
        direction TB
        ZMQ_Pull["📥 ZeroMQ Consumer<br/>(PULL Mode)"]:::compute
        TRT_Eng["🔥 TensorRT Engine (INT8/FP16)<br/>Multi-GAN / Transformer"]:::compute
...
本项目旨在解决复杂深度学习模型（Multi-GAN, Transformer）在资源受限边缘设备上难以实时部署的痛点。通过构建 **RK3568 (Gateway/NPU) + Jetson Orin Nano (Compute/GPU)** 的双机异构集群，HEQIS 实现了算力的分级调度与极致优化。
...
*   **双机协同调度**：
    ...
    *   **Compute Node (Jetson Orin Nano)**: 负责承载重型计算任务，利用 **Ampere 架构的 Tensor Core** 进行并行加速。
*   **极致性能优化**：
    ...
    *   **双端加速**: RK3568 端打通 RKNN 全流程；Jetson Orin 端基于 TensorRT 实现 **INT8/FP16** 精度量化与**结构化稀疏 (Structural Sparsity)** 加速。

### 🎯 算力调度策略 (Workload Partitioning)
本项目的核心设计思想在于将计算任务合理地拆分到最适合的硬件单元上，实现性能与成本的最优平衡。

| 硬件节点           | 承担任务                                       | 技术原因                                                                                             |
| :----------------- | :--------------------------------------------- | :--------------------------------------------------------------------------------------------------- |
| **RK3568 (1 TOPS)** | 1. 数据清洗 & 归一化 (CPU) <br> 2. **技术指标计算 (NPU)** <br> 3. 轻量级特征提取 (NPU) | 作为低功耗网关，其 NPU 适合处理模式固定的**可量化任务**，实现极致**能效比**。                  |
| **Jetson Orin Nano (40 TOPS)** | 1. **Transformer + RoPE (GPU)** <br> 2. **Multi-GAN 博弈 (GPU)** <br> 3. VAE 模块推理 (GPU) | **Ampere 架构与 Tensor Core** 为 Transformer 等 SOTA 模型提供澎湃算力；**成熟 CUDA 生态**确保复杂算子高效执行。 |
...
graph LR
    ...
    B["RK3568 Gateway<br/>(1 TOPS NPU)"]:::dev
    C["Jetson Orin Nano<br/>(40 TOPS GPU)"]:::dev
...
*   **Edge Node 2 (Jetson Orin Nano)**: **JetPack 5.x+** / Ubuntu 20.04
...
### Phase 2: 模型转换与量化
*   **For RK3568 (NPU)**: 使用 `rknn-toolkit2` 将轻量级特征提取模型转为 `.rknn` (FP16/INT8)。
*   **For Jetson Orin (GPU)**: 使用 `trtexec` 或 Python API 将计算密集型模型转为 `.engine` (FP16/INT8)。
    ```bash
    # 示例：Jetson Orin 端转换
...
### Phase 3: 分布式推理启动
1.  **启动 Jetson Orin 计算节点** (Consumer):
    ```bash
...
2.  **启动 RK3568 网关节点** (Producer):
    ...
    ```
*此时，RK3568 将通过 ZeroMQ 将预处理后的 Tensor 流式传输至 Jetson Orin，并实时接收返回的交易信号。*
